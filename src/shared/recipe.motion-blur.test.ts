...
// 완전한 레시피(배경/배지/커서 등)를 유도해 두고, zoomSegments/trim만 갈아 끼워 시나리오를 만든다.
...
function withSegment(seg: ZoomSegment, durationMs = 10000): RenderRecipe {
  return { ...base, durationMs, trim: { startMs: 0, endMs: durationMs }, zoomSegments: [seg] }
}
...
describe('모션 블러 서브프레임 샘플링: 전환 구간에만, 정지 구간엔 0', () => {
  ...
  it('트림 밖 시각은 null', () => {
    const trimmed: RenderRecipe = { ...zoomRecipe, trim: { startMs: 2000, endMs: 4000 } }
